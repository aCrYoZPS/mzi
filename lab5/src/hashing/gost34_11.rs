...
impl Gost34_11 {
    ...
    fn p(a: &[u8]) -> Vec<u8> {
        let mut output = Vec::from([0u8; BLOCK_SIZE]);
    ...
    fn s(a: &[u8]) -> Vec<u8> {
        ...
        return a.iter().map(|&x| PI[x as usize]).collect();
    }
    ...
    fn add_mod_512(a: &[u8], b: &[u8]) -> Vec<u8> {
        debug_assert_eq!(a.len(), BLOCK_SIZE);
    ...
    fn final_hash(mut h: Vec<u8>, n: Vec<u8>, sigma: Vec<u8>) -> Vec<u8> {
        let zero = [0u8; BLOCK_SIZE];

        h = Self::g(&zero, &n, &h);
        h = Self::g(&zero, &sigma, &h);
    ...
    pub fn hash_512(bytes: &[u8], output_len: usize) -> Vec<u8> {
        ...
        while end >= BLOCK_SIZE {
            ...
            let m = &bytes[start..end];

            h = Self::g(&n, m, &h);
            n = Self::add_mod_512(&n, &Self::from_u64(512));
